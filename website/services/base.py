...
class Service(object):
    ...
    def get(self, id, user=None):
        ...
        if user:
            query = self._add_user_condition(query, user)
        return query.filter(self.__model__.id == id).one()

    def get_all(self, user=None, **kwargs):
        query = self.__model__.query
        query = self.filter(query, **kwargs)
        if user:
            query = self._add_user_condition(query, user)
        query = query.order_by(self.__model__.id.desc())
        return query

    def get_all_ids(self, *ids, **kwargs):
    ...
    def filter(self, query, **kwargs):
        ...
        for k, v in kwargs.items():
            attr = getattr(self.__model__, k)
            # if searched by a string run a LIKE comparison
            if isinstance(v, str):
                query = query.filter(attr.like(v + '%'))
            # if searched by an int do a direct lookup or
            # objects can always be filtered dirrectly too
            else:
